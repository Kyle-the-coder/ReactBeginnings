...
//Pc does the same as xbox, see XboxView.jsx for comments
...
const PcResults = () => {
    ...
    useEffect(() => {
        ...
        let call2 = [];
        let call3 = [];
        axios.get('https://api.rawg.io/api/games?parent_platforms=1&dates=2015-01-01,2023-01-01&page_size=50&page=1&')
            .then(res => {
                call1 = res.data.results

                axios.get('https://api.rawg.io/api/games?parent_platforms=1&dates=2015-01-01,2023-01-01&page_size=50&page=2&')
                    .then(res => {
                        call2 = res.data.results

                        axios.get('https://api.rawg.io/api/games?parent_platforms=1&dates=2015-01-01,2023-01-01&page_size=50&page=3&')
                            .then(res => {
    ...
    console.log("sorted is: ", sortedArr(totalRes))

    return (
        <div className='mainC1'>
            <div className="infoBox">
                <h4 className='px-3 text-white my-0'><strong>Top results for "PC"</strong></h4>
                <Link className='text-decoration-none text-white backLink2' to="/game/platforms">Back to platforms</Link>
            </div>


            <div className='mainC'>
                {sortedArr(totalRes).length == 0 ?
